...
export default function useUpdateProfilePic({ profile_pic, cover_pic, history }) {
  const [loading, setLoading] = useState(false)
  const { userState, userDispatch } = useContext(UserContext)
  const { uiDispatch } = useContext(UIContext)

  const updateProfilePic = () => {
    let filename = `profile_pic/${userState.currentUser.name}-${Date.now()}-${profile_pic.name}`
