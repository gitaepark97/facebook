...
export const UserReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'REMOVE_ACCOUNT':
      let accountArray = state.recentAccounts.filter(account => account.id !== action.payload)
      let accountsData = []
      accountsData = localStorage.accounts ? JSON.parse(localStorage.accounts) : []
    ...
    case 'REMOVE_FRIENDS_REQUEST_SENDED':
      let rvs_f_filtered = state.sendedFriendRequests.filter(r => r.id !== action.payload)
      return {
        ...state,
    ...
    case 'ADD_FRIENDS_REQUEST_RECEIVED':
      return {
        ...state,
        receivedFriendRequests: [action.payload, ...state.receivedFriendRequests],
      }

    case 'REMOVE_FRIENDS_REQUEST_RECEIVED':
      let rv_f_filtered = state.receivedFriendRequests.filter(r => r.id !== action.payload)
      return {
        ...state,
