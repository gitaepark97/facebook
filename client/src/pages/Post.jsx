...
export default function Post() {
  ...
  const isContent = () => {
    return postState.post.body.feelings || postState.post.body.at || postState.post.body.date
  }
        ...
        <Grid container spacing={3}>
          <Grid item md={7} sm={12} xs={12}>
            ...
            >
                ...
                <CardHeader
                  avatar={
                    postState.post.user.profile_pic ? (
                      <Avatar>
                        <img style={{ width: '100%', height: '100%' }} src={postState.post.user.profile_pic} alt="" />
                      </Avatar>
                    ) : (
                      <AvartarText
                  ...
                  title={
                    postState.post && <Typography style={{ fontWeight: '800' }}>{postState.post.user.name}</Typography>
                  }
              ...
              {postState.post.profilePostData && Object.keys(postState.post.profilePostData).length ? (
                <>
                  <CardMedia
                  ...
                  >
                    <img src={postState.post.profilePostData.profileImage} width="100%" height="100%" alt="" />
                  </Avatar>
                </>
              ) : null}
            </Card>
          </Grid>
          <Grid style={{ marginBottom: !uiState.mdScreen ? '70px' : '0px' }} item md={5} sm={12} xs={12}>
            <Paper
              style={{
                padding: '16px',
                backgroundColor: uiState.darkMode && 'rgb(36,37,38)',
            ...
            {postState.post.comments && postState.post.comments.length ? (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  {postState.post.commentPagination.totalPage === postState.post.commentPagination.currentPage ? (
                    <Typography variant="h6" color="primary">
                      No more comments
                    </Typography>
                  ) : (
                    <Button variant="contained" color="primary" onClick={handleFetchComments}>
                      More Comments
                    </Button>
                  )}
                </div>
